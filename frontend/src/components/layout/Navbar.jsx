...
/**
 * Navbar — persistent global top bar for the app shell.
 *
 * Renders the SAME header structure/spacing/height for every visitor,
 * authenticated or not, so pages like public profiles never look like a
 * separate application:
 *
 * Left:  app logo / wordmark (always shown)
 * Right: settings icon · avatar dropdown — only shown when logged in.
 *        Logged-out visitors see a "Login" link instead, pointing at the
 *        existing /login route. No other auth-only controls are shown
 *        to them.
 */
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/[0.06] bg-gray-950/80 backdrop-blur-md">
      <div className="mx-auto max-w-6xl px-6 h-14 flex items-center justify-between gap-4">
        ...
        {/* Right actions — differ by auth state */}
        {user ? (
          ...
          <Link
            to="/login"
            className="px-3.5 py-1.5 rounded-lg text-sm font-medium text-white bg-amber-600/[0.1] hover:bg-amber-600/[0.2] border border-white/[0.08] transition-colors"
          >
            Login
          </Link>
        )}
